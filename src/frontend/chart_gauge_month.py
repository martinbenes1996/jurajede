...
def create_plot(history_size):
    ...
    km_past_mean = x[x.month == month_now]\
        ...
        .km\
        .mean()
    if km_past_mean < km_now:
        threshold = {'line': {'color': "white", 'width': 4}, 'thickness': 1, 'value': km_past_mean}
    else:
        threshold = {}
    # create plot
    fig = go.Figure(
        go.Indicator(
            ...
            title = {'text': f"Měsíc v historii {month_now_name}"},#f"Month in {month_now_name} history"},
...
def register(app):
    @app.callback(
        dash.dependencies.Output('gauge-monthly', 'figure'),
        dash.dependencies.Input('history-size-slider', 'value'),
        prevent_initial_call=True
    )
    def get_plot(index):
        print("chart_gauge_month.get_plot()")
